...
When you visit a supported movie page (IMDb, TMDB, Allociné) or use the right-click lookup:
...
Ratearr makes requests to three third-party services:
...
- **Settings** (enabled sources, cache TTL) — stored in `browser.storage.sync`.
